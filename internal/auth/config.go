...
// GetCredentialStoreAccessTokens retrieves the JWT that
//
//	Desktop uses for API sessions.
...
// readUserCreds reads user credentials from the config file for a given registry entry
...
// getAccessTokenConfigKey generates the config key for access tokens (used by  Desktop)
...
// GetCredentialStorePullTokens is a convenience function - use ConfigStore.GetCredentialStorePullTokens instead
func GetCredentialStorePullTokens(registryEntry string) (string, string, error) {
	store := NewConfigStore()
	return store.GetCredentialStorePullTokens(registryEntry)
}

// GetCredentialStoreAccessTokens is a convenience function - use ConfigStore.GetCredentialStoreAccessTokens instead
func GetCredentialStoreAccessTokens(registryEntry string) (string, string, error) {
	store := NewConfigStore()
	return store.GetCredentialStoreAccessTokens(registryEntry)
}

// GetUserCreds is an alias for GetCredentialStorePullTokens for backward compatibility
func GetUserCreds(registryEntry string) (string, string, error) {
	return GetCredentialStorePullTokens(registryEntry)
}
